...
class bondIssuanceContract extends Contract {
    ...
    async SignRequestSPV(ctx,tokenID,typeID, buyRequestID,sellRequestID,sellRequestPrice,sellRequestBackedBy,sellRequestOwner){
        ...
        if ( sellRequest.Price!==parseInt(sellRequestPrice) || sellRequest.BackedBy!==sellRequestBackedBy || sellRequest.Owner!==sellRequestIdentity){
            throw new Error(`Sell Request has been modified`)
        }

        value.Receiver=sellRequestIdentity;

        // const new_iterator=await ctx.stub.getStateByPartialCompositeKey('SellRequest',[userID]);
        ...
        if (sellRequest.IsSigned){
            throw new Error(`User ID ${new_value.Receiver} has signed his/her request`);
        }
        ...
        const n_valueID= 'Request';
        const valueBuffer= Buffer.from(JSON.stringify(n_value));
        await ctx.stub.putState(n_valueID,valueBuffer);
        await ctx.stub.deleteState(value.ID);
        await ctx.stub.deleteState(new_value.ID);
    ...
    async SignRequestOriginator(ctx, poolID,fullRequestID,buyRequestIdentity,buyRequestAmount,buyRequestPrice){
        ...
        const tokenID= await ctx.stub.createCompositeKey('POOL',[poolID]);
        ...
        if (pool.ppaIDs.length!==value.value.Amount){
            throw new Error(`Size of Pool and size of Buy Request do not match`);
        }
        if (value.new_value.IsSigned){
            throw new Error(`Already signed`);
        }
        value.new_value.IsSigned=true;
        if (buyRequestPrice!==value.buyRequest.Price || buyRequestAmount!==value.buyRequest.Amount || buyRequestIdentity!==value.buyRequest.Owner){
            throw new Error('Some args of Full Request have been modified')
        }
        // DvP is triggered
        await this._dvp(ctx,value.TokenID,pool.poolID,value.value.Price);
    ...
    async Rating(ctx,poolID){
        ...
        const key=await ctx.stub.createCompositeKey('Rating');
        const value={
            ID: key,
            BackedBy: poolID,
            Owner: userID,
    ...
    async IssueBond(ctx){
        ...
        const ratingBytes= await ctx.stub.getState('Rating');
        ...
        valueBuffer=Buffer.from(JSON.stringify(bondToken));
...
class servicingContract extends Contract {
    ...
    async beforeTransaction(ctx) {
        let userMSPID=ctx.GetMSPID();
        if (userMSPID!=='investorMSP' && userMSPID!=='spvMSP' && userMSPID!=='ratingagency'){
            throw new Error(`Your organization ${userMSPID} is not allowed to perform this action`);
        }
        let isClient=await ctx.GetRole();
        console.log(`isClient: ${isClient}`);
    ...
    async BidOrder(ctx, amount, price,tokenID,identificativo,date){
        ...
        const bookAsk=await this._sortingBidOrders(ctx,bookOrder[1])
