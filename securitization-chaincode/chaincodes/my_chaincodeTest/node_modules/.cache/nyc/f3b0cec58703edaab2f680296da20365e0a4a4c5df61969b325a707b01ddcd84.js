...
class commonSecuritizationContract extends Contract {
    ...
    */
    ...
    async beforeTransaction(ctx) {
        // let getTxTimestamp=await ctx.GetTxTimestamp();
...
class ppaContract extends Contract {
    ...
    async _isPPAProposed(ctx,userID){
        const iterator=await ctx.stub.getStateByPartialCompositeKey('PPA',[userID]);
        let results=[];
        for await (const res of iterator){
            results.push(res.key)
        }
    ...
    async QueryPPARequest(ctx){
        let iterator=await ctx.stub.getStateByPartialCompositeKey("PPARequest",[])
        let results=[];
        for await (const res of iterator) {
    ...
    async PPAProposal(ctx,PPARequestClientCert,PPARequestGenericID){
        ...
        const date=await ctx.stub.getTxTimestamp();
        const day=1;
        const month=10;
        const year= 2032
    ...
    async AcceptPPA(ctx,ppaID){
        ...
        let date=await ctx.stub.getTxTimestamp();
    ...
    // consumption
    async EnergyConsumptionPerPPA(ctx,ppaID,energyConsumption){
...
class bondIssuanceContract extends Contract {
    ...
    async BuyRequest(ctx,amount,price, poolID){
        const userID=await ctx.GetUserIdentity('cif');

        const genericID=await ctx.stub.getTxID();
        const requestID=await ctx.stub.createCompositeKey('BuyRequest',[userID,genericID]);

        let poolCompositeKey=await ctx.stub.createCompositeKey('POOL',[poolID]);
    ...
    async QueryFullRequest(ctx){
        let iterator=await ctx.stub.getStateByPartialCompositeKey('Request')
    ...
    async Rating(ctx,poolID,rating){
        const userID= await ctx.GetUserIdentity('cif');
    ...
    async IssueBond(ctx,ratingID,p){
        ...
        let ThisDate=await ctx.stub.getTxTimestamp();
...
class servicingContract extends Contract {
    ...
    async _transferCashTokens(ctx,bidInputs,askInput,orderBook){
        ...
        let referencePrice=askInput.Price
        for (let i=0; i<bidInputs.length;i++){
            ...
            if (bidInputs[i].FullMatch){
                ...
                lastBid= myBid[0]
                lastQuantity=myBid.Quantity-myBid.QuantityMatch
                lastQuantityMatch=0
                lastRestingQuantity=myBid.Quantity-myBid.QuantityMatch
        ...
        for (let j=0;j<usedOwners.length;j++){
            ...
            outputBondToken.ID=await ctx.stub.createCompositeKey('BondToken',[usedOwners[j],outputBondToken.GenericID,':0']);
        ...
        let securitizationCoin1={
            ID: await ctx.stub.createCompositeKey('securitizationCoin',[submitterID,transactionID,':0']),
            GenericID: transactionID,
            TypeID: ':0',
            Issuer: await ctx.clientIdentity.getIDBytes(),
            Owner: submitterID,
        ...
        if(restingQuantity!==0){
            ...
            let securitizationCoin2={
                ID: await ctx.stub.createCompositeKey('securitizationCoin',[lastOwner,transactionID,':1']),
                GenericID: transactionID,
                TypeID: ':1',
                Issuer: await ctx.clientIdentity.getIDBytes(),
                Owner: lastOwner,
            ...
            let indice=bids.indexOf(thisBid)
            bids[indice]=lastBid;
            orderBook=[bids,asks]
        ...
        if ((askInput.Quantity-askInput.QuantityMatch)!==0 && !askInput.MarketPrice){
            outputBondToken.ID=await ctx.stub.createCompositeKey('BondToken',[userID,outputBondToken.GenericID,':1']);
            outputBondToken.Amount=bondToken.Amount-bondAcumulator;
            let outputBondTokenBuffer= Buffer.from(JSON.stringify(outputBondToken))
            await ctx.stub.putState(outputBondToken.ID,outputBondTokenBuffer)
            let thisAsk=await asks.filter(item => item.ID===askInput.ID);
            let index=asks.indexOf(thisAsk);
            thisAsk.Quantity=thisAsk.Quantity-thisAsk.QuantityMatch;
            thisAsk.QuantityMatch=0;
            thisAsk.TokenID=outputBondToken.ID;       
            asks[index]=thisAsk;
            orderBook=[bids,asks]
            await ctx.stub.deleteState(bondToken.ID)
        }else if ((askInput.Quantity-askInput.QuantityMatch)===0 && !askInput.MarketPrice){
            let thisAsk=await asks.filter(item => item.ID!==askInput)
            orderBook=[bids,thisAsk,[referencePrice]]
        }else{
            orderBook=[bids,asks]
        }

        // if(partialMatched.length>1){
        //     throw new Error('Error')
        // }
    ...
    async TransferCoupons(ctx, bondTokenID,TypeID, spvID){
        ...
        let iterator=await ctx.stub.getStateByPartialCompositeKey("securitizationCoin",[spvID]);
    ...
    async _transferBondTokens(ctx,askInputs,bidInput, orderBook){
        ...
        let referencePrice=bidInput.Price
        
        for (let i=0; i<askInputs.length;i++){
            if (referencePrice>askInputs[i].Price && !bidInput.MarketPrice){
            ...
            if (askInputs[i].FullMatch){
                ...
                lastAskQuantity=myAsk.Quantity-myAsk.QuantityMatch;
                lastAskQuantityMatch=0;
                lastAskRestingQuantity=myAsk.Quantity-myAsk.QuantityMatch;
                // New token ID
        ...
        for (let j=0;j<usedOwners.length;j++){
            ...
            //TODO
            // check coupons received
            outputSecuritizationCoin.ID=await ctx.stub.createCompositeKey('securitizationCoin',[usedOwners[j],outputSecuritizationCoin.GenericID,':0']);
        ...
        let bondToken1={
            ID: await ctx.stub.createCompositeKey('BondToken',[securitizationCoin.Owner,transactionID,':0']),
            GenericID: transactionID,
            TypeID: ':0',
            Rating: token.Rating,
        ...
        if(restingQuantity!==0){
            ...
            let bondToken2={
                ID: await ctx.stub.createCompositeKey('BondToken',[lastOwner,transactionID,':1']),
                GenericID: transactionID,
                TypeID: ':1',
                Rating: token.Rating,
                CouponRate: token.CouponRate,
            ...
            let indice=asks.indexOf(thisAsk)
            asks[indice]=lastAsk;
            orderBook=[bids,asks]
        ...
        if ((bidInput.Quantity-bidInput.QuantityMatch)!==0 && !bidInput.MarketPrice){
            outputSecuritizationCoin.ID=await ctx.stub.createCompositeKey('securitizationCoin',[userID,outputSecuritizationCoin.GenericID,':1']);
            outputSecuritizationCoin.Amount=token.Amount-cashAcumulator;
            let outputSecuritizationCoinBuffer= Buffer.from(JSON.stringify(outputSecuritizationCoin))
            await ctx.stub.putState(outputSecuritizationCoin.ID,outputSecuritizationCoinBuffer)
            let thisBid=await bids.filter(item => item.ID===bidInput.ID);
            let index=bids.indexOf(thisBid);
            thisBid.Quantity=thisBid.Quantity-thisBid.QuantityMatch;
            thisBid.QuantityMatch=0;
            thisBid.TokenID=outputSecuritizationCoin.ID;       
            bids[index]=thisBid;
            orderBook=[bids,asks,[referencePrice]]
            await ctx.stub.deleteState(token.ID)
        }else if ((bidInput.Quantity-bidInput.QuantityMatch)===0 && !bidInput.MarketPrice){
            let thisBid=await bids.filter(item => item.ID!==bidInput)
