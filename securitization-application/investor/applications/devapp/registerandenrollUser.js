...
async function main() {
    try {
        ...
        const wallet = await Wallets.newFileSystemWallet('./investor/applications/my_application/wallet/');
        ...
        const ca = new FabricCAServices(connectionProfile['certificateAuthorities'][`rootca.investor.securitization.com`].url);
        ...
        const my_wallet = await Wallets.newFileSystemWallet('./investor/applications/APPandUI/wallet/'+identityLabel);
        ...
        let pathToFile = path.resolve(require('os').homedir(), 'go/src/github.com/hyperledger/fabric-samples/securitization-network/tfm-network/fabric-ca/investor/users/'+identityLabel+'/');
